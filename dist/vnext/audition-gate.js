...
exports.audition = audition;
exports.ruleQualityPass = ruleQualityPass;
exports.applyRuleQualityGate = applyRuleQualityGate;
const critics_1 = require("./critics");
function audition(plan, cfg = {
 ...
 */
function ruleQualityPass(plan) {
    const THRESH = {
        arc: 0.40, motif: 0.35, contour: 0.35, stepLeap: 0.35, range: 0.5,
        harmony: 0.4, rhythm: 0.4
    };
    const m = (0, critics_1.scoreMelody)(plan);
    const h = (0, critics_1.scoreHarmony)(plan);
    const r = (0, critics_1.scoreRhythm)(plan);
    const ok = m.arc >= THRESH.arc &&
...
function applyRuleQualityGate(plan, issues) {
    const q = ruleQualityPass(plan);
    plan.__quality = q; // attach for logging/response
    if (!q.ok) {
