...
exports.scoreMelody = scoreMelody;
exports.scoreHarmony = scoreHarmony;
exports.scoreRhythm = scoreRhythm;
function scoreMelody(plan) {
...
function scoreRhythm(plan) {
    ...
    const durations = rhythmEvents.map(e => e.duration || 0.25);
...
// HELPER FUNCTIONS
// =============================================================================
function calculateMotifRecurrence(pitches) {
...
function calculateContourEntropy(deltas) {
    if (deltas.length === 0)
        return 0;
...
function calculateStepLeapRatio(deltas) {
    if (deltas.length === 0)
        return 0;
...
function calculateRangeOk(pitches) {
    if (pitches.length === 0)
        return 0;
...
function calculateNarrativeFlow(pitches, deltas) {
    // Measure overall melodic coherence and direction
    const directionChanges = deltas.slice(1).filter((d, i) => (d > 0) !== (deltas[i] > 0)).length;
    const flowScore = 1 - (directionChanges / Math.max(1, deltas.length));
    return Math.max(0, Math.min(1, flowScore));
}
function detectGamingPatterns(pitches, deltas) {
...
function calculateProgressionLegality(chords) {
...
function calculateVoiceLeading(harmonyEvents) {
    // Simplified voice leading analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateHarmonicTension(chords) {
    // Simplified tension analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateHarmonicComplexity(chords) {
    // Simplified complexity analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateHarmonicResolution(chords) {
    // Simplified resolution analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateSyncopation(onsets) {
    // Simplified syncopation analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateGroove(durations, onsets) {
    // Simplified groove analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateTempoScore(durations) {
    // Simplified tempo analysis
    return Math.random() * 0.8 + 0.2; // Placeholder
}
function calculateRhythmicDiversity(durations) {
    // Simplified diversity analysis
    const uniqueDurations = new Set(durations);
    return Math.min(1, uniqueDurations.size / 8);
}
function calculateAccentPattern(rhythmEvents) {
