...
function loadSampleSnapshots(count = 100) {
    const snapshotsFile = path_1.default.resolve(process.cwd(), 'datasets', 'snapshots.jsonl');
    const content = fs_1.default.readFileSync(snapshotsFile, 'utf8');
...
function mockEvaluation(snapshot) {
...
function runSanityCheck() {
    ...
    for (let i = 0; i < snapshots.length; i++) {
        ...
        Object.keys(gateResults.calibrated).forEach(gate => {
            if (gateResults.calibrated[gate]) {
                perGatePasses.calibrated[gate]++;
            }
            if (gateResults.strict[gate]) {
                perGatePasses.strict[gate]++;
            }
        });
