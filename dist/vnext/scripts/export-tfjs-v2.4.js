...
exports.exportTFJSV24 = exportTFJSV24;
exports.generateIntegrityMap = generateIntegrityMap;
exports.generatePreloadManifest = generatePreloadManifest;
exports.validateTFJSFormat = validateTFJSFormat;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const crypto = __importStar(require("crypto"));
function generateIntegrityMap(modelDir) {
    const integrityMap = {};
...
function generatePreloadManifest(modelDir, modelVersion) {
...
function validateTFJSFormat(modelDir) {
    console.log('🔍 Validating TFJS Layers format...');
    const requiredFiles = ['model.json', 'weights_manifest.json'];
    const hasRequiredFiles = requiredFiles.every(file => fs.existsSync(path.join(modelDir, file)));
    if (!hasRequiredFiles) {
...
async function exportTFJSV24(sourceDir, targetDir) {
    ...
    fs.writeFileSync(path.join(targetDir, 'weights_manifest.json'), JSON.stringify(modelJson.weightsManifest, null, 2));
    ...
    fs.writeFileSync(path.join(targetDir, 'integrity.json'), JSON.stringify(integrityMap, null, 2));
    // Generate preload manifest
    const preloadManifest = generatePreloadManifest(targetDir, 'v2.4');
    fs.writeFileSync(path.join(targetDir, 'preload.json'), JSON.stringify(preloadManifest, null, 2));
    // Generate checksums
    ...
    fs.writeFileSync(path.join(hashesDir, 'tfjs-v2.4.export.hash'), JSON.stringify(checksums, null, 2));
    console.log('\n📊 Export Summary:');
