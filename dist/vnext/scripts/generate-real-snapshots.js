...
exports.generateRealSnapshots = generateRealSnapshots;
...
function generateLocationCandidates(count) {
    const candidates = [];
    ...
    if (lat !== undefined && lon !== undefined) {
        try {
            ...
            if (timezone && moment.tz.zone(timezone)) {
                ...
                return swe.swe_julday(ut.year(), ut.month() + 1, ut.date(), ut.hour() + ut.minute() / 60 + ut.second() / 3600, swe.SE_GREG_CAL);
            }
        }
        catch (error) {
...
function calcPositions(jd, includeExtras = true) {
    ...
    for (const [name, id] of Object.entries(PLANETS)) {
        try {
            ...
            let lon = result.longitude || ((_a = result.xx) === null || _a === void 0 ? void 0 : _a[0]);
            if (lon === undefined) {
    ...
    if (includeExtras) {
        for (const [name, id] of Object.entries(EXTRAS)) {
            try {
                ...
                let lon = result.longitude || ((_b = result.xx) === null || _b === void 0 ? void 0 : _b[0]);
                if (lon === undefined) {
                    console.warn(`No longitude found for ${name}`);
                    continue;
                }
...
function calcPlacidusCusps(jd, lat, lon) {
    try {
        ...
        return cusps.map((cusp) => cusp % 360);
    }
    catch (error) {
...
function calcMoonPhase(jd) {
    ...
    try {
        ...
        const sunLon = sunResult.longitude || ((_a = sunResult.xx) === null || _a === void 0 ? void 0 : _a[0]);
        const moonLon = moonResult.longitude || ((_b = moonResult.xx) === null || _b === void 0 ? void 0 : _b[0]);
...
function calcDominantElements(positions) {
    ...
    for (const [name, lon] of Object.entries(positions)) {
        ...
        if (sign === 0 || sign === 4 || sign === 8)
            elementCounts.fire++; // Aries, Leo, Sagittarius
        else if (sign === 1 || sign === 5 || sign === 9)
            elementCounts.earth++; // Taurus, Virgo, Capricorn
        else if (sign === 2 || sign === 6 || sign === 10)
            elementCounts.air++; // Gemini, Libra, Aquarius
        else if (sign === 3 || sign === 7 || sign === 11)
            elementCounts.water++; // Cancer, Scorpio, Pisces
    }
    // Calculate weights
...
function generateSnapshot(candidate) {
...
function generateChartHash(snapshot) {
    ...
    return (0, chartHash_1.generateChartHashSync)(chartData).slice(0, 12);
}
// Main generation function
async function generateRealSnapshots(targetCount = 1000) {
    ...
    for (let i = 0; i < selectedSnapshots.length; i++) {
        ...
        const feat = Array.from((0, feature_encode_1.encodeFeatures)(snapshot));
...
function saveSnapshots(records, outputPath) {
...
if (require.main === module) {
    ...
    const outputPath = process.argv[3] || path_1.default.join(__dirname, '../../../datasets/snapshots.jsonl');
